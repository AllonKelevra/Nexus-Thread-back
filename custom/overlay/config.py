...
class Settings(BaseSettings):
    ...
    # Manual SBP payment method
    MANUAL_PAYMENT_ENABLED: bool = Field(default=False, description="Enable manual SBP payment method")
    MANUAL_DISPLAY_NAME: str = Field(default="По номеру телефона через СБП", description="Display name for manual SBP method")
    ...
    def is_manual_enabled(self) -> bool:
        return bool(self.MANUAL_PAYMENT_ENABLED)

    def get_manual_display_name(self) -> str:
        return (self.MANUAL_DISPLAY_NAME or "").strip() or "По номеру телефона через СБП"
