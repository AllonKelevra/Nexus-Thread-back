...
CUSTOM_DESIGN_ENABLED_KEY = 'CABINET_CUSTOM_DESIGN_ENABLED'
...
class CustomDesignEnabledResponse(BaseModel):
    """Nexus Thread custom design enabled setting."""

    enabled: bool = False


class CustomDesignEnabledUpdate(BaseModel):
    """Request to update Nexus Thread custom design setting."""
...
async def is_superadmin_user(db: AsyncSession, user: User) -> bool:
    """Return True only for RBAC Superadmin-level users."""
    permissions, role_names, role_level = await UserRoleCRUD.get_user_permissions(db, user.id)
    return '*:*' in permissions or 'Superadmin' in role_names or role_level >= SUPERADMIN_LEVEL
...
@router.get('/custom-design', response_model=CustomDesignEnabledResponse)
async def get_custom_design_enabled(
    db: AsyncSession = Depends(get_cabinet_db),
):
    """
    Get Nexus Thread custom design setting.
    This is public so every client can choose the same global design.
    """
    raw = await get_setting_value(db, CUSTOM_DESIGN_ENABLED_KEY)
    enabled = str(raw).strip().lower() in {'true', '1', 'yes', 'on'}
    return CustomDesignEnabledResponse(enabled=enabled)


@router.patch('/custom-design', response_model=CustomDesignEnabledResponse)
async def update_custom_design_enabled(
    payload: CustomDesignEnabledUpdate,
    admin: User = Depends(require_permission('settings:edit')),
    db: AsyncSession = Depends(get_cabinet_db),
):
    """Update Nexus Thread custom design setting. Superadmin only."""
    if not await is_superadmin_user(db, admin):
        raise HTTPException(
            status_code=status.HTTP_403_FORBIDDEN,
            detail='Superadmin access required',
        )

    await set_setting_value(db, CUSTOM_DESIGN_ENABLED_KEY, 'true' if payload.enabled else 'false')

    logger.info(
        'Superadmin updated custom design setting',
        telegram_id=admin.telegram_id,
        enabled=payload.enabled,
    )

    return CustomDesignEnabledResponse(enabled=payload.enabled)
