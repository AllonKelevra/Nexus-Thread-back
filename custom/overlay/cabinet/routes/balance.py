...
async def create_topup(
    ...
    try:
        if request.payment_method == 'yookassa':
            ...
            _pending = {
                'user_id': user.id,
                'bank_key': request.payment_option or '',
                'bank_name': bank_display,
                'amount_kopeks': request.amount_kopeks,
                'amount_rubles': amount_rubles,
            }
            await _cache.set(f'sbp_pending:{_token}', _json.dumps(_pending, ensure_ascii=False), expire=86400)

            _bank_enc = _quote(bank_display)
            payment_url = f'{settings.CABINET_URL.rstrip("/")}/sbp.html?tok={_token}&bank={_bank_enc}&amount={int(amount_rubles)}'
            payment_id  = f'manual_{user.id}_{int(time.time())}'
...
class SbpTicketRequest(BaseModel):
    amount_kopeks: int = Field(..., gt=0)
    bank: str = Field(..., min_length=1)


class YooMoneyTicketRequest(BaseModel):
    amount_kopeks: int = Field(..., gt=0)


class YooMoneyAutoPaymentRequest(BaseModel):
    amount_kopeks: int = Field(..., gt=0)


@router.post('/sbp-ticket', status_code=200)
async def create_sbp_ticket_from_cabinet(
    request: SbpTicketRequest,
    user: User = Depends(get_current_cabinet_user),
    db: AsyncSession = Depends(get_cabinet_db),
):
    """Create a manual SBP support ticket directly from the cabinet."""
    if getattr(user, 'restriction_topup', False):
        raise HTTPException(
            status_code=status.HTTP_403_FORBIDDEN,
            detail='Balance top-up is restricted for this account',
    ...
    if not settings.MANUAL_PAYMENT_ENABLED:
        raise HTTPException(status_code=status.HTTP_400_BAD_REQUEST, detail='Manual SBP payment method is not available')

    methods = await get_payment_methods(user=user, db=db)
    method = next((m for m in methods if m.id == 'manual'), None)
    if not method or not method.is_available:
        raise HTTPException(status_code=status.HTTP_400_BAD_REQUEST, detail='Manual SBP payment method is not available')
    if request.amount_kopeks < method.min_amount_kopeks:
        raise HTTPException(
    ...
    try:
        return await create_sbp_ticket(
            db=db,
            user=user,
            amount_kopeks=request.amount_kopeks,
            bank_key=request.bank,
            notify_admins=True,
        )
...
@router.post('/yoomoney-ticket', status_code=200)
async def create_yoomoney_ticket_from_cabinet(
    request: YooMoneyTicketRequest,
    user: User = Depends(get_current_cabinet_user),
    db: AsyncSession = Depends(get_cabinet_db),
):
    """Create a manual YooMoney support ticket directly from the cabinet."""
    if getattr(user, 'restriction_topup', False):
        raise HTTPException(
            status_code=status.HTTP_403_FORBIDDEN,
            detail='Balance top-up is restricted for this account',
    ...
    if not settings.MANUAL_PAYMENT_ENABLED:
        raise HTTPException(status_code=status.HTTP_400_BAD_REQUEST, detail='YooMoney payment method is not available')

    methods = await get_payment_methods(user=user, db=db)
    method = next((m for m in methods if m.id == 'yoomoney_donate'), None)
    if not method or not method.is_available:
        raise HTTPException(status_code=status.HTTP_400_BAD_REQUEST, detail='YooMoney payment method is not available')
    if request.amount_kopeks < method.min_amount_kopeks:
        raise HTTPException(
    ...
    from app.services.yoomoney_manual_service import create_yoomoney_ticket

    try:
        return await create_yoomoney_ticket(
            db=db,
            user=user,
            amount_kopeks=request.amount_kopeks,
            notify_admins=True,
...
@router.post('/yoomoney-auto-payment', status_code=200)
async def create_yoomoney_auto_payment_from_cabinet(
    request: YooMoneyAutoPaymentRequest,
    user: User = Depends(get_current_cabinet_user),
    db: AsyncSession = Depends(get_cabinet_db),
):
    """Create a pending YooMoney automatic payment form with a unique label."""
    if getattr(user, 'restriction_topup', False):
        raise HTTPException(
            status_code=status.HTTP_403_FORBIDDEN,
            detail='Balance top-up is restricted for this account',
    ...
    from app.services.yoomoney_manual_service import create_yoomoney_auto_payment
...
@router.post('/yoomoney-notification', status_code=200)
async def yoomoney_notification(
    request: Request,
    db: AsyncSession = Depends(get_cabinet_db),
):
    """Public YooMoney HTTP notification endpoint for automatic top-ups."""
    form = await request.form()
    params = dict(form)

    from app.services.yoomoney_manual_service import (
        YooMoneyBalanceCreditFailed,
        YooMoneyNotificationSecretMissing,
        YooMoneyNotificationSignatureInvalid,
        process_yoomoney_notification,
    )

    try:
        return await process_yoomoney_notification(db=db, params=params)
    except YooMoneyNotificationSecretMissing as error:
        raise HTTPException(status_code=status.HTTP_503_SERVICE_UNAVAILABLE, detail=str(error))
    except YooMoneyNotificationSignatureInvalid as error:
        raise HTTPException(status_code=status.HTTP_400_BAD_REQUEST, detail=str(error))
    except YooMoneyBalanceCreditFailed as error:
        raise HTTPException(status_code=status.HTTP_500_INTERNAL_SERVER_ERROR, detail=str(error))


@router.post('/sbp-confirm/{token}', status_code=200)
async def confirm_sbp_payment(token: str, db: AsyncSession = Depends(get_cabinet_db)):
    ...
    raw = await _cache.getdel(f'sbp_pending:{token}')
    if not raw:
        raise HTTPException(status_code=status.HTTP_404_NOT_FOUND, detail='Token expired or already used')

    data = _json.loads(raw)
    fresh_user = await get_user_by_id(db, data['user_id'])
    if not fresh_user:
        raise HTTPException(status_code=status.HTTP_404_NOT_FOUND, detail='User not found')

    amount_kopeks = int(data.get('amount_kopeks') or float(data['amount_rubles']) * 100)
    try:
        return await create_sbp_ticket(
            db=db,
            user=fresh_user,
            amount_kopeks=amount_kopeks,
            bank_key=data.get('bank_key') or '',
            notify_admins=True,
        )
    except ValueError as error:
        raise HTTPException(status_code=status.HTTP_400_BAD_REQUEST, detail=str(error))
...
@router.post('/sbp-admin-confirm/{ticket_id}', status_code=200)
async def admin_confirm_sbp_payment(
    ticket_id: int,
    user: User = Depends(get_current_cabinet_user),
    db: AsyncSession = Depends(get_cabinet_db),
):
    """Admin: confirm SBP payment and credit user balance."""
    if not settings.is_admin(
        telegram_id=getattr(user, 'telegram_id', None),
        email=getattr(user, 'email', None) if getattr(user, 'email_verified', False) else None,
    ):
    ...
    actor = getattr(user, 'username', None) or getattr(user, 'email', None) or str(user.id)
    try:
        return await confirm_sbp_payment(
            db=db,
            ticket_id=ticket_id,
            actor_label=f'cabinet:{actor}',
            source='кабинет',
        )
    except SbpPaymentAlreadyConfirmed as error:
        raise HTTPException(status_code=status.HTTP_409_CONFLICT, detail=str(error))
    except SbpPaymentConfirmInProgress as error:
        raise HTTPException(status_code=status.HTTP_409_CONFLICT, detail=str(error))
    except SbpPaymentNotFound as error:
        raise HTTPException(status_code=status.HTTP_404_NOT_FOUND, detail=str(error))
    except SbpBalanceCreditFailed as error:
        raise HTTPException(status_code=status.HTTP_500_INTERNAL_SERVER_ERROR, detail=str(error))


@router.post('/yoomoney-admin-confirm/{ticket_id}', status_code=200)
async def admin_confirm_yoomoney_payment(
    ticket_id: int,
    user: User = Depends(get_current_cabinet_user),
    db: AsyncSession = Depends(get_cabinet_db),
):
    """Admin: confirm YooMoney payment and credit user balance minus 3% fee."""
    if not settings.is_admin(
        telegram_id=getattr(user, 'telegram_id', None),
        email=getattr(user, 'email', None) if getattr(user, 'email_verified', False) else None,
    ):
    ...
    from app.services.yoomoney_manual_service import (
        YooMoneyBalanceCreditFailed,
        YooMoneyPaymentAlreadyConfirmed,
        YooMoneyPaymentConfirmInProgress,
        YooMoneyPaymentNotFound,
        confirm_yoomoney_payment,
    )

    actor = getattr(user, 'username', None) or getattr(user, 'email', None) or str(user.id)
    try:
        return await confirm_yoomoney_payment(
            db=db,
            ticket_id=ticket_id,
            actor_label=f'cabinet:{actor}',
            source='кабинет',
        )
    except YooMoneyPaymentAlreadyConfirmed as error:
        raise HTTPException(status_code=status.HTTP_409_CONFLICT, detail=str(error))
    except YooMoneyPaymentConfirmInProgress as error:
        raise HTTPException(status_code=status.HTTP_409_CONFLICT, detail=str(error))
    except YooMoneyPaymentNotFound as error:
        raise HTTPException(status_code=status.HTTP_404_NOT_FOUND, detail=str(error))
    except YooMoneyBalanceCreditFailed as error:
        raise HTTPException(status_code=status.HTTP_500_INTERNAL_SERVER_ERROR, detail=str(error))
    except ValueError as error:
...
@router.get('/ticket-actions/{ticket_id}', status_code=200)
async def get_ticket_actions(
    ...
    if not settings.is_admin(
        ...
        return {
            'has_sbp_confirm': False,
            'status': 'missing',
            'has_yoomoney_confirm': False,
            'yoomoney_status': 'missing',
        }

    from app.utils.cache import cache as _act_cache
    sbp_meta = await _act_cache.get(f'sbp_meta:{ticket_id}')
    yoomoney_meta = await _act_cache.get(f'yoomoney_meta:{ticket_id}')

    sbp_status = 'missing'
    has_sbp_confirm = False
    if sbp_meta:
        if sbp_meta.get('confirmed', False):
            sbp_status = 'confirmed'
        else:
            sbp_status = 'pending'
            has_sbp_confirm = True

    yoomoney_status = 'missing'
    has_yoomoney_confirm = False
    if yoomoney_meta:
        if yoomoney_meta.get('confirmed', False):
            yoomoney_status = 'confirmed'
        else:
