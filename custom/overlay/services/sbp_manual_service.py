...
def normalize_bank(bank: str | None) -> tuple[str, str]:
    """Return safe bank key and display name."""
    bank_key = (bank or '').strip().lower()
    if bank_key not in BANK_DISPLAY:
        raise ValueError('Invalid SBP bank')
    bank_name = BANK_DISPLAY[bank_key]
    return bank_key, bank_name.replace(' (рекомендуется)', '')
...
async def create_sbp_ticket(
    *,
    ...
    _, bank_display = normalize_bank(bank_key)
