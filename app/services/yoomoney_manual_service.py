...
from app.services.custom_payment_settings_service import (
    ...
    fee_percent,
    get_provider_config,
    resolve_yoomoney_notification_secret,
)
...
def _amount_to_kopeks(value: str | float | Decimal | None) -> int:
    if value is None:
        return 0
    return int((Decimal(str(value).replace(',', '.')) * Decimal(100)).quantize(Decimal(1)))


async def _verify_notification_sign(db: AsyncSession, params: dict[str, Any]) -> None:
    try:
        secret = await resolve_yoomoney_notification_secret(db)
    except CustomPaymentSecretError as error:
        raise YooMoneyNotificationSecretMissing(str(error)) from error
...
async def create_yoomoney_auto_payment(
    *,
    ...
    config = await get_provider_config(db, 'yoomoney_donate')
    receiver = config['receiver_wallet']
    fee_basis_points = config['fee_basis_points']
    if not receiver:
        raise ValueError('YooMoney receiver wallet is not configured')
    credit_amount_kopeks = calculate_credit_amount(amount_kopeks, fee_basis_points)
    meta = {
        'user_id': user.id,
        ...
        'fee_percent': float(fee_percent(fee_basis_points)),
...
async def _notify_admins(
    ...
    text = (
        ...
        f'Комиссия: <b>{fee_percent(fee_basis_points):.2f}%</b>\n\n'
        f'⏰ {dt_moscow}'
    )
...
async def create_yoomoney_ticket(
    *,
    ...
    body = (
        ...
        f'Сумма платежа: <b>{amount_kopeks / 100:.0f} ₽</b>\n'
        f'К зачислению: <b>{credit_amount_kopeks / 100:.0f} ₽</b>\n'
        f'Комиссия сервиса: <b>{fee_percent(fee_basis_points):.2f}%</b>\n\n'
    ...
    if notify_admins:
        await _notify_admins(ticket.id, user, amount_kopeks, credit_amount_kopeks, fee_basis_points, dt_moscow)
...
async def confirm_yoomoney_payment(
    *,
    ...
    config = await get_provider_config(db, 'yoomoney_donate')
    fee_basis_points = int(meta.get('fee_basis_points', config['fee_basis_points']))
    credit_amount_kopeks = calculate_credit_amount(gross_amount_kopeks, fee_basis_points)
    ...
    message_text = (
        ...
        f'Сумма доната: {gross_amount_kopeks / 100:.0f} ₽, '
        f'комиссия YooMoney: {fee_percent(fee_basis_points):.2f}%.'
    )
    if ticket:
...
async def process_yoomoney_notification(
    *,
    ...
    await _verify_notification_sign(db, params)
    ...
    config = await get_provider_config(db, 'yoomoney_donate')
    fee_basis_points = int(meta.get('fee_basis_points', config['fee_basis_points']))
    expected_credit_kopeks = int(
        meta.get('credit_amount_kopeks') or calculate_credit_amount(gross_amount_kopeks, fee_basis_points)
    )
